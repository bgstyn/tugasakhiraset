...
@section('page_title', 'Edit Bundle')

@section('content')
<div class="max-w-5xl mx-auto space-y-6 animate-[fadeIn_0.4s_ease-out_forwards]">

    {{-- Back --}}
    <a href="{{ route('bundles.show', $bundle) }}" class="inline-flex items-center gap-2 text-xs font-semibold text-slate-400 hover:text-white transition">
        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
        Kembali ke Detail Bundle
    </a>

    <form action="{{ route('bundles.update', $bundle) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $bundle->name) }}" required maxlength="200"
                ...
                {{-- Kode Bundle (readonly) --}}
                <div>
                    <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Kode Bundle</label>
                    <div class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-indigo-400 text-sm font-mono font-bold">
                        {{ $bundle->code }}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="location_id"
                        ...
                        @foreach($locations as $loc)
                            <option value="{{ $loc->id }}" {{ old('location_id', $bundle->location_id) == $loc->id ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="2" maxlength="1000"
                        class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-slate-200 text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/30 resize-none transition">{{ old('description', $bundle->description) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-6 space-y-4">
            ...
            <div class="space-y-4 max-h-[480px] overflow-y-auto pr-1" id="assetList">
                ...
                <div class="asset-category-group">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        <label for="asset_{{ $asset->id }}"
                            ...
                            data-name="{{ strtolower($asset->name . ' ' . $asset->asset_id . ' ' . $asset->category) }}"
                            data-category="{{ addslashes($category) }}">
                            <input type="checkbox" id="asset_{{ $asset->id }}" name="asset_ids[]" value="{{ $asset->id }}"
                                {{ in_array($asset->id, old('asset_ids', $selectedIds)) ? 'checked' : '' }}
                            ...
                            <div class="overflow-hidden flex-1 min-w-0">
                                ...
                                <p class="text-[11px] text-slate-500 font-mono mt-0.5">{{ $asset->asset_id }}</p>
                                <div class="flex items-center gap-2 mt-1 flex-wrap">
                                    ...
                                    @if(in_array($asset->id, $selectedIds))
                                        <span class="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">✓ Sudah dipilih</span>
                                    @endif
                                </div>
                            </div>
                        </label>
        ...
        <div class="flex gap-3 justify-end">
            <a href="{{ route('bundles.show', $bundle) }}" class="px-6 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-sm font-semibold transition">Batal</a>
